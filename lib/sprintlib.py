...
def is_valid_sprint_address(address, network='mainnet'):
    ...
    sprint_version = 140 if network == 'testnet' else 63
    ...
    try:
        ...
        # rescue from exception, not a valid Sprint address
        return False

    if (address_version != sprint_version):
        return False
...
# shims 'til we can fix the sprintd side
def SHIM_serialise_for_sprintd(sentinel_hex):
    from models import SPRINTD_GOVOBJ_TYPES
    # unpack
    ...
    # add 'type' attribute
    obj[1]['type'] = SPRINTD_GOVOBJ_TYPES[govtype]

    # superblock => "trigger" in sprintd
    if govtype == 'superblock':
        obj[0] = 'trigger'

    # sprintd expects an array (even though there is only a 1:1 relationship between govobj->class)
    obj = [obj]

    # re-pack
    sprintd_hex = serialise(obj)
    return sprintd_hex


# shims 'til we can fix the sprintd side
def SHIM_deserialise_from_sprintd(sprintd_hex):
    from models import SPRINTD_GOVOBJ_TYPES

    # unpack
    obj = deserialise(sprintd_hex)
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('sprint.conf')
