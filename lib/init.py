...
def has_sprint_conf():
    ...
    valid_sprint_conf = False

    # ensure sprint_conf exists & readable
    #
    # if not, print a message stating that Sprint Core must be installed and
    # configured, including JSONRPC access in sprint.conf
    try:
        f = io.open(config.sprint_conf)
        valid_sprint_conf = True
    except IOError as e:
        print(e)

    return valid_sprint_conf
...
def main():
    ...
    if not has_sprint_conf():
        print("SprintCore must be installed and configured, including JSONRPC access in sprint.conf")
        sys.exit(1)
