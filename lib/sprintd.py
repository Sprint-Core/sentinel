...
class SprintDaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some sprintd methods
    ...
    @classmethod
    def from_sprint_conf(self, sprint_dot_conf):
        from sprint_config import SprintConfig
        config_text = SprintConfig.slurp_config_file(sprint_dot_conf)
        creds = SprintConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_current_masternode_vin(self):
        from sprintlib import parse_masternode_status_vin
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = sprintlib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = sprintlib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = sprintlib.blocks_to_seconds(diff)
    ...
    def has_sentinel_ping(self):
        ...
        return (getinfo['protocolversion'] >= config.min_sprintd_proto_version_with_sentinel_ping)
