...
class Watchdog(BaseModel, GovernanceClass):
    ...
    govobj_type = SPRINTD_GOVOBJ_TYPES['watchdog']
    only_masternode_can_submit = True

    @classmethod
    def active(self, sprintd):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at >= (now - sprintd.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
        return resultset

    @classmethod
    def expired(self, sprintd):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at < (now - sprintd.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
    ...
    def is_expired(self, sprintd):
        now = int(time.time())
        return (self.created_at < (now - sprintd.SENTINEL_WATCHDOG_MAX_SECONDS))

    def is_valid(self, sprintd):
        if self.is_expired(sprintd):
            return False

        return True

    def is_deletable(self, sprintd):
        if self.is_expired(sprintd):
            return True
